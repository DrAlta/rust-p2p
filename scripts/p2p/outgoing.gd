...
signal first_answer_generated(dict_answer: Dictionary)
...
func on_answer_generated(dict_answer):
	logy("trace", "[outgoming:48]on_answer_generated(dict_answer)")
	if answer == "":
			emit_signal("first_answer_generated", {"ID" : dict_answer.ID, "OfferID": offer_id, "Answer" : dict_answer.Answer, "ICE" : link.ice})
...
func on_new_ice_candidate(id, mid_name, index_name, sdp_name):
	logy("trace", "[outgoing:56]on_new_ice_candidate(id, mid_name, index_name, sdp_name)")
	emit_signal("new_ice_candidate", id, offer_id, mid_name, index_name, sdp_name)
	if answer != "":
		emit_signal("answer_generated", {"ID" : id, "OfferID": offer_id, "Answer" : answer, "ICE" : link.ice})
